...
export async function sendNewEnquiryNotifications(DB, env, reference) {
  ...
  const adminOk = await deliverNotification(DB, env, {
    ...
    text: `A new enquiry has been received.\n\nReference: ${reference}\nCustomer: ${enquiry.name}\nEmail: ${enquiry.email}\nTelephone: ${enquiry.telephone || "Not provided"}\nSubject: ${enquiry.subject}\nCategory: ${enquiry.category}\nBooking reference: ${enquiry.booking_reference || "Not provided"}\nOrder reference: ${enquiry.order_reference || "Not provided"}\n\n${enquiry.message}\n\nAdministrator: https://experiences.jagroupservices.co.uk/admin/?section=enquiries&reference=${encodeURIComponent(reference)}`,
    html: brandedEmail("New customer enquiry", ["A new customer enquiry has been received.", `Open it in the Administrator Control Centre: https://experiences.jagroupservices.co.uk/admin/?section=enquiries&reference=${encodeURIComponent(reference)}`], adminDetails)
  });
  const customerOk = await deliverNotification(DB, env, {
    ...
    text: `Hello ${enquiry.name},\n\nThank you for contacting JA Experiences & Discovery. We have received your enquiry.\n\nReference: ${reference}\nExpected response time: within 2 working days.\n\nView My Enquiries: https://experiences.jagroupservices.co.uk/account/enquiries/`,
    html: brandedEmail("We have received your enquiry", [`Hello ${enquiry.name},`, "Thank you for contacting us. We have received your enquiry and normally respond within 2 working days.", `Your reference is ${reference}.`, "View My Enquiries: https://experiences.jagroupservices.co.uk/account/enquiries/"])
  });
...
export async function updateEnquiryAsAdmin(DB, env, body, identity) {
  ...
  if (reply) {
    ...
    const ok = await deliverNotification(DB, env, {
      ...
      subject: `${reference}: Reply from JA Experiences & Discovery`,
      text: `Hello ${current.name},\n\n${reply}\n\nStatus: ${status}\nReference: ${reference}\n\nContinue the conversation: https://experiences.jagroupservices.co.uk/account/enquiries/`,
      html: brandedEmail("We have replied to your enquiry", [`Hello ${current.name},`, reply, `Status: ${status}`, `Reference: ${reference}`, "Continue the conversation: https://experiences.jagroupservices.co.uk/account/enquiries/"])
    });
    ...
    await deliverNotification(DB, env, {
      ...
      html: brandedEmail("Your enquiry status has changed", [`Hello ${current.name},`, `The status of your enquiry is now ${status}.`, `Reference: ${reference}`, "View My Enquiries: https://experiences.jagroupservices.co.uk/account/enquiries/"])
    });
  }
...
export async function addCustomerReply(DB, env, reference, email, message) {
  ...
  const ok = await deliverNotification(DB, env, {
    ...
    html: brandedEmail("Customer reply received", [`${thread.enquiry.name} replied to enquiry ${reference}.`, reply, `Open in the Administrator Control Centre: https://experiences.jagroupservices.co.uk/admin/?section=enquiries&reference=${encodeURIComponent(reference)}`])
  });
  await DB.prepare(`UPDATE enquiry_messages SET notification_status = ? WHERE id = ?`).bind(ok ? "Sent" : "Failed", messageId).run();
