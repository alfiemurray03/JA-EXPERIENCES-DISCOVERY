...
export default async function handler(req: Request, res: Response) {
  ...
  const result: Record<string, unknown> = {
    ...
    priceIds: {
      personal:         pricePersonal        ? { id: pricePersonal,        set: true, label: 'Explore Plan' }                 : { set: false, label: 'Explore Plan' },
      standard:         priceStandard        ? { id: priceStandard,        set: true, label: 'Plan Plan' }                    : { set: false, label: 'Plan Plan' },
      professional:     priceProfessional    ? { id: priceProfessional,    set: true, label: 'Complete Plan' }                : { set: false, label: 'Complete Plan' },
      org_starter:      priceOrgStarter      ? { id: priceOrgStarter,      set: true, label: 'Together Plan' }                : { set: false, label: 'Together Plan' },
