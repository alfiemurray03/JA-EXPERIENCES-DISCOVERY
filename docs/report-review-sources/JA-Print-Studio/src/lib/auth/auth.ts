...
export function getAuth() {
  ...
  const auth = betterAuth({
    ...
    // Base URL — required for BetterAuth callbacks and redirects
    baseURL: process.env.BETTER_AUTH_URL || 'https://jnjwm2s1cy.preview.c37.airoapp.ai',
    ...
    emailAndPassword: {
      enabled: true,
      sendResetPassword: async ({ user: _user, url, token: _token }) => {
        // SMTP is not yet configured — log the reset URL to the server console
        // so it can be used during development/testing.
        // Once email is wired up, replace this with your email sending logic.
        console.log('[JA Print Studio] Password reset requested for:', _user.email);
        console.log('[JA Print Studio] Reset URL:', url);
        console.warn('[JA Print Studio] ⚠️  Email not sent — SMTP is not configured. Use the URL above to reset the password.');
      },
    },
